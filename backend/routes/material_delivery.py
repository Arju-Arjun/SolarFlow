...
material_delivery_bp = Blueprint("material_delivery_bp", __name__, url_prefix="/api/material-delivery")
...
def create_or_update_material_delivery(customer_id):
    ...
    if material_delivery:
        ...
        material_delivery = MaterialDelivery(
            customer_id=customer_id, changes=changes, extra_material=extra_material,
            structure_changes=structure_changes, electrical_delivered=electrical_delivered,
            structure_delivered=structure_delivered, panel_delivered=panel_delivered, comments=comments
        )
...
def get_material_delivery(customer_id):
    ...
    return jsonify({
        "id": material_delivery.id, "customer_id": material_delivery.customer_id,
        "changes": material_delivery.changes, "extra_material": material_delivery.extra_material,
        "structure_changes": material_delivery.structure_changes, "electrical_delivered": material_delivery.electrical_delivered,
        "structure_delivered": material_delivery.structure_delivered, "panel_delivered": material_delivery.panel_delivered,
        "comments": material_delivery.comments, "created_at": material_delivery.created_at.isoformat() if material_delivery.created_at else None,
...
def delete_material_delivery(customer_id):
    ...
    db.session.commit()
    return jsonify({"message": "Material delivery record deleted successfully"}), 200