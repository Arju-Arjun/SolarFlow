...
const sectionDefinitions = [
  ...
  },
  {
    key: "mnre",
    label: "MNRE Profile",
    getData: (data) => data.mnreProfile || data.mnre || data.mnre_profile,
    requiredFields: (data) =>
      !data?.enabled ? [] : ["mnre_status", "feasibility_file", "ack_file"],
  },
  {
  ...
  },
  {
    ...
    requiredFields: (data) =>
      !data?.enabled
        ? []
        : ["ack_file", "status", "submission", "first_payment", "second_payment"],
  },
];
...
const getMissingFields = (section, workflowData) => {
  ...
  return requiredFields.filter((field) => !isTruthyField(data[field]));
};
